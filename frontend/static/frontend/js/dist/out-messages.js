...
!function() {
/*!****************************************!*\
  !*** ./static/frontend/js/messages.js ***!
  \****************************************/
var received_ = document.querySelector('#received_');
var sent_ = document.querySelector('#sent_');
var deleted_ = document.querySelector('#deleted_');
var content = document.getElementById("content");
var send_ = document.querySelector('#send_');

var hash = __webpack_require__(/*! object-hash */ "./node_modules/object-hash/dist/object_hash.js");

var getReceivedMessages = function getReceivedMessages() {
  content.innerHTML = "";
  ...
  fetch(url = '../api/messages/received', {
    ...
    var wiadomosci = data.data;
    table = document.createElement("table");
    table.className = "striped";
    table.innerHTML = "<thead>\n" + "          <tr>\n" + "              <th>Temat</th>\n" + "              <th>Nadawca</th>\n" + "              <th>Data</th>\n" + "          </tr>\n" + "        </thead>" + "        <tbody>" + "                " + "        </tbody>";
    content.append(table);
    wiadomosci.forEach(function (wiadomosc) {
      var tbody = document.getElementsByTagName("tbody")[0];
      wiadomoscRow = tbody.insertRow();
      temat = wiadomoscRow.insertCell();
      temat.innerHTML = "<span id=\"".concat(wiadomosc.Id, "\">").concat(wiadomosc.Temat, "</span>");
      wiadomoscRow.appendChild(temat);
      nadawca = wiadomoscRow.insertCell();
      nadawca.innerHTML = "<span>".concat(wiadomosc.Nadawca.Name, "</span>");
      wiadomoscRow.appendChild(nadawca);
      dataWyslania = wiadomoscRow.insertCell();
      dataWyslania.innerHTML = "<span>".concat(wiadomosc.Data, "</span>");
...
var getSentMessages = function getSentMessages() {
  content.innerHTML = "";
  cookies_data = sessionStorage.getItem('cookies_data');
  csrfcookie_ = sessionStorage.getItem('csrfcookie');
  fetch(url = '../api/messages/sent', {
    ...
    var wiadomosci = data.data; // haha spaghetti code goes brrr
    ...
    wiadomosci.forEach(function (wiadomosc) {
      ...
      wiadomoscRow.appendChild(temat);
      nadawca = wiadomoscRow.insertCell();
      nadawca.innerHTML = "<span>".concat(wiadomosc.Adresaci[0], "</span>");
      wiadomoscRow.appendChild(nadawca);
      dataWyslania = wiadomoscRow.insertCell();
      dataWyslania.innerHTML = "<span>".concat(wiadomosc.Data, "</span>");
      wiadomoscRow.appendChild(dataWyslania);
...
var getDeletedMessages = function getDeletedMessages() {
  content.innerHTML = "";
  cookies_data = sessionStorage.getItem('cookies_data');
  csrfcookie_ = sessionStorage.getItem('csrfcookie');
...
var getAddressee = function getAddressee() {
  cookies_data = sessionStorage.getItem('cookies_data');
  csrfcookie_ = sessionStorage.getItem('csrfcookie');
  fetch(url = '../api/messages/recipients', {
    ...
    data.addressee.data.forEach(function (recipient) {
      var recipient_hash = "".concat(hash.MD5(recipient));
...
var sendMessage = function sendMessage() {
  var recipient_hash_ = document.querySelector('#recipients').value;
  var subject_ = document.querySelector('#subject').value;
  var content_ = document.querySelector('#message-content').value;
  var recipient_ = JSON.parse(sessionStorage.getItem(recipient_hash_));
...
var getMessageContent = function getMessageContent(event) {
  send_data = {
    'cookies_data': cookies_data,
...
/******/ })()
;
//# sourceMappingURL=out-messages.js.map